...
const HomePage = () => {
  ...
  const getTotal = async () => {
    try {
      const { data } = await apiService.get("/product/product-count");
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
    }
  ...
  const filterProduct = async () => {
    try {
      const { data } = await apiService.post("/product/product-filters", {
        checked,
        radio,
      });
      setProducts(data?.products);
          ...
          <div className="d-flex flex-wrap">
            {products?.map((p) => (
              <div className="card m-2" key={p._id}>
                <img
                  src={`https://kind-pink-nightingale-sock.cyclic.app/product/product-photo/${p._id}`}
                ...
                <div className="card-body">
                  ...
                  <div className="card-name-price">
                    ...
                    <button
                      className="btn btn-dark ms-1 px-1"
                      onClick={() => {
                        setCart([...cart, p]);
